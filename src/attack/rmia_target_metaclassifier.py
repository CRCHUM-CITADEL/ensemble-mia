...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "challenge_points_repo",
        type=str,
        help="Path to the repo of all challenge csvs.",
    )
    ...
    master_challenge_df = (
        pd.concat(dfs, ignore_index=True).drop_duplicates().dropna(subset=["trans_id"])
    )
    master_challenge_df_1 = (
        pd.concat(dfs_1, ignore_index=True)
        .drop_duplicates()
        .dropna(subset=["trans_id"])
    )
    ...
    print(
        "Length of potential train population excluding all ids in challenge sets: ",
        len(train_pop),
    )
    ...
    master_challenge_labels = np.where(
        master_challenge_df["trans_id"].isin(unique_ids_1), 1, 0
    )
    master_challenge_labels = pd.DataFrame(
        master_challenge_labels, columns=["is_train"]
    )
    master_challenge_labels.to_csv(
        os.path.join(new_folder, "master_challenge_labels.csv")
    )
    ...
    # set config
    shutil.copyfile(
        trans_domain_file_path, os.path.join(new_folder, "trans_domain.json")
    )
    shutil.copyfile(
        dataset_meta_file_path, os.path.join(new_folder, "dataset_meta.json")
    )
