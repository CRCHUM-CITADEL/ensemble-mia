...
sys.modules["midst_competition.multi_table_ClavaDDPM"] = sys.modules[
    "midst_models.multi_table_ClavaDDPM"
]
sys.modules["midst_competition.single_table_ClavaDDPM"] = sys.modules[
    ...
    def find_class(self, module, name):
        ...
        if module.startswith("midst_competition.multi_table_ClavaDDPM"):
            module = module.replace(
                "midst_competition.multi_table_ClavaDDPM",
                "midst_models.multi_table_ClavaDDPM",
                1,
            )
...
def clava_synthesizing(
    ...
    if all_exist:
        ...
        for key in table_names:
            ...
            synthetic_tables[key] = pd.read_csv(
                os.path.join(table_dir, f"{key}_synthetic.csv")
            )
    ...
    if os.path.exists(os.path.join(save_dir, "before_matching/synthetic_tables.pkl")):
        print("Synthetic tables found, loading...")
        synthetic_tables = pickle.load(
            open(os.path.join(save_dir, "before_matching/synthetic_tables.pkl"), "rb")
        )
    ...
    for parent, child in relation_order:
        ...
        if parent is None:
            ...
            generated_final_df = generated_final_df[
                table_info[(parent, child)]["columns"]
            ]
...
def clava_eval(tables, save_dir, configs, relation_order, synthetic_tables=None):
    ...
    report = gen_multi_report(configs["general"]["data_dir"], syn_data_path, "clava")
